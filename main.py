...
from data_preprocessing.text_process.text_process import *
from dataset import *
from model.fcmf import *
from model.resnet_utils import *
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        help="List of predefined Aspect.",
    )
    parser.add_argument(
        "--num_polarity", default=4, type=int, help="Number of sentiment polarity."
    )
    parser.add_argument("--num_imgs", default=7, type=int, help="Number of images.")
    parser.add_argument("--num_rois", default=4, type=int, help="Number of RoIs.")
    parser.add_argument(
        "--image_dir", default="../vimacsa/image", help="path to images"
    )
    ...
    parser.add_argument(
        "--ddp", action="store_true", help="local_rank for distributed training on gpus"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--fine_tune_cnn", action="store_true", help="fine tune pre-trained CNN if True"
    )
    parser.add_argument(
        "--add_layer", action="store_true", help="whether to add another encoder layer"
    )
    ...
    if master_process:
        print(
            "===================== RUN Fine-grained Cross-modal Fusion ====================="
        )
        logging.basicConfig(
            filename="training_fcmf.log",
    ...
    if args.train_batch_size == 0:
        raise ValueError(
            "train_batch_size must greater than gradient_accumulation_steps"
        )
    ...
    try:
        ...
        raise ValueError(
            "Get image/roi aspect category first. Please run run_image_categories.py or run_roi_categories.py"
        )

    if args.do_train:
        train_data = pd.read_json(f"{args.data_dir}/train.json")
        dev_data = pd.read_json(f"{args.data_dir}/dev.json")
        train_data["comment"] = train_data["comment"].apply(
            lambda x: normalize_class.normalize(text_normalize(convert_unicode(x)))
        )
        dev_data["comment"] = dev_data["comment"].apply(
            lambda x: normalize_class.normalize(text_normalize(convert_unicode(x)))
        )

        num_splitted_train = train_data.shape[0] // ddp_world_size
        train_data = train_data.iloc[
            num_splitted_train * ddp_local_rank : num_splitted_train * ddp_local_rank
            + num_splitted_train,
            :,
        ]
    ...
    resnet_img = myResNetImg(
        resnet=img_res_model, if_fine_tune=args.fine_tune_cnn, device=device
    )
    resnet_roi = myResNetRoI(
        resnet=roi_res_model, if_fine_tune=args.fine_tune_cnn, device=device
    )
    ...
    if args.do_train:
        ...
        dev_dataloader = DataLoader(
            dev_dataset, sampler=dev_sampler, batch_size=args.eval_batch_size
        )
        ...
        for train_idx in range(int(args.num_train_epochs)):
                    ...
                    with torch.autocast(
                        ...
                        with torch.no_grad():
                            ...
                            for img_idx in range(args.num_imgs):
                                img_features = (
                                    resnet_img(t_img_features[:, img_idx, :])
                                    .view(-1, 2048, 49)
                                    .permute(0, 2, 1)
                                    .squeeze(1)
                                )  # batch_size, 49, 2048
                                ...
                                for roi_idx in range(args.num_rois):
                                    roi_features = resnet_roi(
                                        roi_img_features[:, img_idx, roi_idx, :]
                                    ).squeeze(1)  # batch_size, 1, 2048
                                    roi_f.append(roi_features)

                                roi_f = torch.stack(roi_f, dim=1)
                                encoded_roi.append(roi_f)

                            encoded_img = torch.stack(
                                encoded_img, dim=1
                            )  # batch_size, num_img, 49, 2048
                            encoded_roi = torch.stack(
                                encoded_roi, dim=1
                            )  # batch_size, num_img, num_roi, 49,2048
                        ...
                        if args.gradient_accumulation_steps > 1:
                            all_asp_loss = (
                                all_asp_loss / args.gradient_accumulation_steps
                            )
            ...
            if master_process:
                ...
                for step, batch in enumerate(
                    tqdm(dev_dataloader, position=0, leave=True, desc="Evaluating")
                ):
                ...
                if eval_f1 >= max_f1:
                    ...
                    if args.do_train:
                        save_model(
                            f"{args.output_dir}/seed_{args.seed}_fcmf_model.pth",
                            model,
                            train_idx,
                        )
                        save_model(
                            f"{args.output_dir}/seed_{args.seed}_resimg_model.pth",
                            resnet_img,
                            train_idx,
                        )
                        save_model(
                            f"{args.output_dir}/seed_{args.seed}_resroi_model.pth",
                            resnet_roi,
                            train_idx,
                        )
    ...
    if args.do_eval and (not args.ddp or torch.distributed.get_rank() == 0):
        ...
        resimg_checkpoint = load_model(
            f"{args.output_dir}/seed_{args.seed}_resimg_model.pth"
        )
        resroi_checkpoint = load_model(
            f"{args.output_dir}/seed_{args.seed}_resroi_model.pth"
        )
        ...
        for step, batch in enumerate(
            tqdm(test_dataloader, position=0, leave=True, desc="Evaluating")
        ):
            (
                t_img_features,
                roi_img_features,
